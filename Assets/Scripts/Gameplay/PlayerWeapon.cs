...
namespace ShootingGallery.Gameplay
{
    ...
    public class PlayerWeapon : NetworkBehaviour
    {
        ...
        // directly rather than guessing blind). Built up in stages, each verified by rendering
        // alone before touching the prefab again: Euler(-90,0,0) for a recognizable profile,
        // rolled 90 more for a natural "holding it" grip-low pose, then yawed 45 degrees so the
        // barrel points downrange (away into the screen) like an aimed firearm instead of lying
        // flat in profile - a full 90 degree yaw looks straight down the barrel and is exactly
        // the "scrambled" edge-on view we were trying to get away from in the first place; 45 is
        // the point where it still reads clearly as a gun while visibly pointing away. None of
        // this reduces to simple per-axis Euler addition (confirmed the hard way, repeatedly), so
        // this is the precomputed equivalent of Quaternion.Euler(0,-45,0) * Quaternion.Euler(0,0,-90)
        // * Quaternion.Euler(-90,0,0).
        private static readonly Vector3 RevolverOrientationCorrection = new Vector3(0f, 45f, 270f);
